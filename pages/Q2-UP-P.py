...
a2_up = str(st.session_state["a2_up_p"])
b2_up = str(st.session_state["b2_up_p"])
Q2_UP_Text = st.session_state["Q2_UP_p"].replace('a', a2_up)
Q2_UP_Text = Q2_UP_Text.replace('b', b2_up)

st.header("고난도 문제 연습 1번")
st.subheader(Q2_UP_Text)
answer1 = st.number_input("답 : ", key="1", value=None, placeholder="답을 입력하세요.", format="%f")


if "B7_p" not in st.session_state:
    st.session_state["B7_p"] = False

def make1():
    st.session_state["B7_p"] = True

st.write("고난도 문제는 채점 기회가 1번 뿐이니 신중하게 눌러주세요.")
if st.button("채점하기", key="a", on_click=make1, disabled=st.session_state["B7_p"]):
    if answer1 == None:
        answer1 = 0
    if float(answer1) == float(st.session_state["x2_up_p"]):
        st.write("정답입니다.")
    else:
        st.write("아쉽지만 오답입니다.")
        st.write("문제를 바탕으로 세운 식을 확인해보면 ", st.session_state["a2_up_p"], "\u00F7", st.session_state["b2_up_p"], "이 됩니다.")
