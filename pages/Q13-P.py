...
if "B47_p1" not in st.session_state:
    st.session_state["B47_p1"] = False
if "B48_p1" not in st.session_state:
    st.session_state["B48_p1"] = False
if "B49_p1" not in st.session_state:
    st.session_state["B49_p1"] = False
if "B50_p1" not in st.session_state:
    st.session_state["B50_p1"] = False
if "B51_p1" not in st.session_state:
    st.session_state["B51_p1"] = False


def make1():
    st.session_state["B46_p1"] = True
def make2():
    st.session_state["B47_p1"] = True
def make3():
    st.session_state["B48_p1"] = True
def make4():
    st.session_state["B49_p1"] = True
def make5():
    st.session_state["B50_p1"] = True
def make6():
    st.session_state["B51_p1"] = True
...
if st.button("채점하기", key="a", on_click=make1, disabled=st.session_state["B46_p1"]):
    ...
    if float(answer1) == float(st.session_state["x13_p1"]):
        ...
        st.session_state["Q13_p1"] = st.session_state["Q13_p1"] + 1
...
if st.session_state["Q13_p1"] == 1:
    ...
    if st.button("채점하기", key="b", on_click=make2, disabled=st.session_state["B47_p1"]):    
        ...
        if float(answer2) == float(st.session_state["x13_p1"]):
            ...
            st.session_state["Q13_p1"] = st.session_state["Q13_p1"] + 1.1
       

if st.session_state["Q13_p1"] == 2.1:  
...
if st.session_state["Q13_p1"] == 2.5:
    ...
    if st.button("채점하기", key="d", on_click=make4, disabled=st.session_state["B49_p1"]): 
        ...
        if float(answer4) == float(st.session_state["x13_p1"]):
            ...
            st.session_state["Q13_p1"] = st.session_state["Q13_p1"] - 0.5
        else:
...
if st.session_state["Q13_p1"] == 3.1:  
    ...
    if st.button("채점하기", key="e", on_click=make5, disabled=st.session_state["B50_p1"]): 
        ...
        if int(answer5) == int(st.session_state['y13_2_p1']):
            ...
            st.session_state["Q13_p1"] = st.session_state["Q13_p1"] + 0.9
...
if "B47_p2" not in st.session_state:
    st.session_state["B47_p2"] = False
if "B48_p2" not in st.session_state:
    st.session_state["B48_p2"] = False
if "B49_p2" not in st.session_state:
    st.session_state["B49_p2"] = False
if "B50_p2" not in st.session_state:
    st.session_state["B50_p2"] = False
if "B51_p2" not in st.session_state:
    st.session_state["B51_p2"] = False


def make1_p2():
    st.session_state["B46_p2"] = True
def make2_p2():
    st.session_state["B47_p2"] = True
def make3_p2():
    st.session_state["B48_p2"] = True
def make4_p2():
    st.session_state["B49_p2"] = True
def make5_p2():
    st.session_state["B50_p2"] = True
def make6_p2():
    st.session_state["B51_p2"] = True
...
if st.button("채점하기", key="a_p2", on_click=make1_p2, disabled=st.session_state["B46_p2"]):
    ...
    if float(answer1_p2) == float(st.session_state["x13_p2"]):
        ...
        st.session_state["Q13_p2"] = st.session_state["Q13_p2"] + 1
...
if st.session_state["Q13_p2"] == 1:
    ...
    if st.button("채점하기", key="b_p2", on_click=make2_p2, disabled=st.session_state["B47_p2"]):    
        ...
        if float(answer2_p2) == float(st.session_state["x13_p2"]):
            ...
            st.session_state["Q13_p2"] = st.session_state["Q13_p2"] + 1.1
       

if st.session_state["Q13_p2"] == 2.1:  
    st.write("나머지를 구하는 데 어려움이 있는 것 같군요.")
    st.write("이전처럼 소수의 나눗셈을 자연수로 바꾸면 몫은 유지되지만 나머지는 바뀌기 때문에 다른 방법으로 접근해봅시다.")
    st.write("우선 몫을 자연수 부분까지만 구한 후 답을 입력해봅시다.")
...
if st.session_state["Q13_p2"] == 2.5:
    ...
    if st.button("채점하기", key="d_p2", on_click=make4_p2, disabled=st.session_state["B49_p1"]): 
        ...
        if float(answer4_p2) == float(st.session_state["x13_p2"]):
            ...
            st.write("하지만 더 쉬운 방법으로는 자연수 부분까지 몫을 구하는 과정을 세로셈으로 똑같이 한 후, 나누어지는 수의 소수점을 그대로 내려서 구한 나머지에 소수점을 찍어도 됩니다.")
            st.write("이 사실을 기억하면서 더 연습해봅시다.")
            st.session_state["Q13_p2"] = st.session_state["Q13_p2"] - 0.5
        else:
            st.write("오답입니다. 나머지를 구하는 방식을 이해하지 못하거나 계산 실수가 있었군요.")
            st.write("7을 2로 나누고 남은 나머지를 구할 때 7에서 몫인 3에 2를 곱한 후 빼주듯이")
...
if st.session_state["Q13_p2"] == 3.1:  
    ...
    if st.button("채점하기", key="e_p2", on_click=make5_p2, disabled=st.session_state["B50_p2"]): 
        ...
        if int(answer5_p2) == int(st.session_state['y13_2_p2']):
            ...
            st.session_state["Q13_p2"] = st.session_state["Q13_p2"] + 0.9

if st.session_state["Q13_p2"] == 3.5:
    st.write("그럼 이제 몫의 자연수 부분을 이용하여 나머지를 구해봅시다. 나머지를 구할 때는 몫의 자연수 부분을 구하느라 자연수로 바꾸었던 숫자를 다시 소수로 돌려놓고 생각해야 합니다.")
