...
if st.session_state["Q12"] == 2.5:
    ...
    if st.button("채점하기", key="d", on_click=make4, disabled=st.session_state["B42"]): 
        if float(answer4) == float(st.session_state["x12"]):
            ...
            st.write(right_answer, "를 ", st.session_state['y12_2'], "에서 반올림하면 ", st.session_state['x12'], "이 된답니다.")
...
if st.session_state["Q12"] == 3.1:  
    ...
    a12 = int(st.session_state["a12"]*100)
    b12 = int(st.session_state["b12"]*100)
    ...
    st.write("그렇게 되면 ", st.session_state["a12"], "이 ", a12, "로 바뀌게 되고,")
    st.write(st.session_state["b12"], "이 ", b12, "로 바뀌게 됩니다.")
    st.write("그렇다면 자연수의 나눗셈으로 바꾼 후 몫을", st.session_state['y12_2'], "까지 구한 후 답을 입력해봅시다.")
    Q12_easy_Text_2 = f"12. {a12} \u00F7 {b12}의 {st.session_state['y12']}하시오."
...
if st.session_state["Q12"] == 4:
    ...
    st.write(f"{a12} \u00F7 {b12}의 {st.session_state['y12']}하면 답은 {right_answer}가 됩니다.")
    st.write("이때 ", right_answer, "를 ", st.session_state['y12_2'], "에서 반올림하면 ", st.session_state['x12'], "이 된답니다.")
